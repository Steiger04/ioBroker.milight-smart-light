...
export default {
  ...
  init: function (connOptions, connCallbacks, objectsRequired) {
    ...
    if (document.URL.split('/local/')[ 1 ] || ( typeof socketUrl === 'undefined' && !connOptions.connLink ) ||
        ...
        if (that._connCallbacks.onCommand && id === that.namespace + '.control.command') {
          ...
          if (state.val &&
            ...
            try {
              ...
              Vue.console.log('Command seems to be an object, but cannot parse it: ' + state.val)
            }
          }
  ...
  getStates: function (IDs, callback) {
    ...
    if (this._type === 'local') {
      ...
      if (this.gettingStates > 1) {
        ...
        Vue.console.log('Trying to get empty list, because the whole list could not be loaded')
        IDs = []
      }
      var that = this
