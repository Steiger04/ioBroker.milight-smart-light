const messages = {
    ...
    isAllowedZoneNumber({ $model }, name) {
        return this.$t('validateMsg.isAllowedZoneNumber', { field: name, value: this.$t(`zonesForm.${$model}`) });
    },
    isAllowedZoneType({ $model }, name) {
        return this.$t('validateMsg.isAllowedZoneType', { field: name, value: this.$t(`zonesForm.${$model}`) });
    },
    regexColorOffset({ $model }, name) {
        return this.$t('validateMsg.regexColorOffset', { field: name, value: $model });
    },
    regexGroupAndName({ $model }, name) {
        return this.$t('validateMsg.regexGroupAndName', { field: name, value: $model });
    },
};
