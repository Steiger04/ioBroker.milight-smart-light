...
function mslcommandsV6 (adapter, mslStatestore) {
    ...
    commandsV6.bridge.constructor.prototype.nightMode = _.wrap(commandsV6.bridge.nightMode, async function (func, options) {
        ...
        if (mslStatestore.getState(options.fullChannelPath + 'effectOn').val === true) {
            await mslStatestore.setState({ dp: 'effectOn', val: false, params: options });
            await mslStatestore.setState({ dp: 'effectOff', val: true, params: options });
            await mslStatestore.setState({ dp: 'effectOnOff', val: false, params: options });
        }
    ...
    commandsV6.bridge.constructor.prototype.effectMode = _.wrap(commandsV6.bridge.effectMode, async function (func, options) {
        if (mslStatestore.getState(options.fullChannelPath + 'effectOn').val === false) {
            await mslStatestore.setState({ dp: 'effectOn', val: true, params: options });
            await mslStatestore.setState({ dp: 'effectOff', val: false, params: options });
            await mslStatestore.setState({ dp: 'effectOnOff', val: true, params: options });
        }
