...
function mslcommands2(adapter) {
  ...
  commands2.rgbw.constructor.prototype.on = _.wrap(commands2.rgbw.on, function(func, options, ...argv) {
    ...
    if (options.val === true || options.val === 1) {
      Promise.all([adapter.setStateAsync(_idOff, false, true), adapter.setStateAsync(_idOnOff, true, true)]).catch(function(err) {
        adapter.log.error('commands2:wrapper:rgbw:on->' + err.message);
      });
  ...
  // wrapper rgbw off
  commands2.rgbw.constructor.prototype.off = _.wrap(commands2.rgbw.off, function(func, options, ...argv) {
    if (options === null) {
      return func(argv[0]);
    }
    ...
    if (options.val === true || options.val === 1) {
      Promise.all([adapter.setStateAsync(_idOn, false, true), adapter.setStateAsync(_idOnOff, false, true)]).catch(function(err) {
        adapter.log.error('commands2:wrapper:rgbw:off->' + err.message);
      });
  ...
  commands2.rgbw.constructor.prototype.onoff = function(options) {
    ...
    if (options.val === true || options.val === 1) {
      Promise.all([adapter.setStateAsync(_idOn, true, true), adapter.setStateAsync(_idOff, false, true)]).catch(function(err) {
        adapter.log.error('commands2:wrapper:rgbw:onoff->' + err.message);
      });
