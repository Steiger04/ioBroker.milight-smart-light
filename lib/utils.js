...
function getControllerDir(isInstall) {
  ...
  if (controllerDir[controllerDir.length - 3] == 'adapter') { // jshint ignore:line
    controllerDir.splice(controllerDir.length - 3, 3);
    controllerDir = controllerDir.join('/');
  }
  else if (controllerDir[controllerDir.length - 3] == 'node_modules') { // jshint ignore:line
    controllerDir.splice(controllerDir.length - 3, 3);
    controllerDir = controllerDir.join('/');
...
function getConfig() {
  return JSON.parse(fs.readFileSync(controllerDir + '/conf/iobroker.json')); // jshint ignore:line
}
